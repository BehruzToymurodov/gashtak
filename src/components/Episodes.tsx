			...
			<div className='relative aspect-video overflow-hidden bg-white/5'>
				{!imgError ? (
					...
					<div className='w-full h-full flex items-center justify-center'>
						<span className='font-oswald font-bold text-4xl neon-text-sm opacity-40'>
							EP{ep.ep}
						</span>
					</div>
				)}
				...
				<div className='absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-200'>
					<div className='w-14 h-14 rounded-full border border-neon/60 flex items-center justify-center bg-black/50 backdrop-blur-sm'>
						<svg
							className='w-6 h-6 neon-text-sm ml-1'
							viewBox='0 0 24 24'
							fill='currentColor'
						>
...
export default function Episodes({
	...
	useEffect(() => {
		Promise.allSettled(EPISODES.map(fetchOEmbed)).then(results => {
			const map: Record<string, EpisodeMeta> = {}
			results.forEach((r, i) => {
				if (r.status === 'fulfilled') map[EPISODES[i].youtubeId] = r.value
			})
			setMetas(map)
		})
	}, [])
	...
	const popular = useMemo(
		() =>
			[...EPISODES]
				.sort((a, b) => getEpisodeViews(b) - getEpisodeViews(a))
				.slice(0, 4),
		[],
	)
				...
				{popular.length > 0 && (
					<div className='mb-12'>
						<div className='mb-6 flex items-center gap-4'>
							...
							<span className='font-oswald text-lg uppercase tracking-[0.35em] neon-text'>
								Mashhur podkastlar
							</span>
							<div className='h-px flex-1 max-w-[3rem] bg-neon/40' />
						</div>
